...
describe('if the hook is used multiple times', () => {
  it('returns the same shared error object in case it loads badly', async () => {
    ...
    await expect(headElementMockAppendChild).toHaveBeenLastCalledWith(givenScriptElement);

    await expect(scriptElementAddEventListenerMock).toHaveBeenCalledTimes(2);
    expect(scriptElementAddEventListenerMock).toHaveBeenCalledWith('load', expect.any(Function));
    expect(scriptElementAddEventListenerMock).toHaveBeenCalledWith('error', expect.any(Function));
